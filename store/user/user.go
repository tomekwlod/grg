...
func (u *userStore) FindOne(ctx context.Context, id int64) (*core.User, error) {
	user := new(core.User)

	err := u.db.QueryRowxContext(ctx, "SELECT id, email, password FROM users WHERE id=$1 LIMIT 1", id).StructScan(user)
